...
export function EntitlementManager({
  ...
  async function handleGrant(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/banks/${bankId}/entitlements`, {
      method: "POST",
  ...
  async function handleRevoke(id: string) {
    if (!confirm("Revoke this user's access to the bank?")) return;
  ...
  return (
    <div className="space-y-4">
      ...
      {entitlements.length === 0 ? (
        <p className="text-sm text-muted-foreground">Nobody has access to this bank yet.</p>
      ) : (

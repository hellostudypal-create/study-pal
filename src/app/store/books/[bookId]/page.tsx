        ...
        <div className="flex flex-col gap-6 sm:flex-row">
          {book.coverImageUrl && (
            ...
            <img
              ...
              alt=""
              className="mx-auto h-48 w-48 shrink-0 rounded-lg object-cover shadow-md sm:mx-0 sm:h-56 sm:w-56"
            />
          )}
          <div className="min-w-0 flex-1">
            ...
            {book.description && (
              <Markdown className="mt-4 text-sm text-muted-foreground">{book.description}</Markdown>
            )}
        ...
        {book.chapters.map((chapter, index) => (
          <div key={chapter.id} className="space-y-3">
            <div className="flex items-stretch overflow-hidden rounded-lg border border-border bg-card">
              <div className="relative aspect-square w-28 shrink-0 self-center sm:w-36">
                {chapter.coverImageUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={chapter.coverImageUrl} alt="" className="h-full w-full object-cover" />
                ) : (
                  <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-primary-tint to-primary/5">
              ...
              <div className="min-w-0 flex-1 p-4">
                <span className="rounded-full bg-secondary px-2 py-0.5 text-[11px] font-bold text-secondary-foreground">
                  {t("books.chapterLabel", { number: index + 1 })}
                </span>
                <h3 className="mt-1.5 truncate font-bold">{chapter.title}</h3>
                {chapter.subtitle && (
                  <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{chapter.subtitle}</p>
                )}
                {!chapter.isFreePreview && (
